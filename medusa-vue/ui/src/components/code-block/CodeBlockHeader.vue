...
const updateIndicator = () => {
    ...
    for (let i = 0; i < activeIndex; i++) {
        ...
        if (tab) {
             left += tab.offsetWidth + 12 // 12 is gap? or padding?
             // The reference calculation: 
             // prevTabRef ? (sum of widths + 12 * index) + 15 : 15
             // Actually reference uses reduce.
        }
    }
    
    // Simplified logic: matches reference mostly but might need tuning.
    // Reference:
    /*
